        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:gap-16 items-center">
          <div className="space-y-8">
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              <Button asChild size="lg" className="gap-2.5 rounded-2xl h-13 px-8 glow-primary hover:shadow-lg transition-shadow">
                <Link to="/nicotine-pouches">
                  {t('hero.cta')}
                  <ArrowRight className="h-4 w-4" />
                </Link>
              </Button>
              <Button asChild variant="outline" size="lg" className="rounded-2xl h-13 px-8 border-border/40 hover:border-primary/30 hover:text-primary">
          ...
          <div className="relative hidden lg:block">
            <div className="relative grid grid-cols-2 gap-5">
              {[
                ...
                <div
                  key={item.name}
                  className={`rounded-3xl glass-panel p-5 hover:border-primary/20 transition-all ${i === 1 ? 'mt-10' : i === 3 ? 'mt-10' : ''}`}
                >
                  <div className={`h-${i % 2 === 0 ? 28 : 24} rounded-2xl bg-gradient-to-br ${item.gradient} flex items-center justify-center mb-4`}>
                    <span className={`text-${i % 2 === 0 ? '4xl' : '3xl'}`}>{item.emoji}</span>
                  </div>
