...
type EmptyVariant = 'plp' | 'pdp' | 'cart' | 'search' | 'ops-table' | 'generic';
...
const variantDefaults: Record<EmptyVariant, { icon: React.ElementType; title: string; description: string }> = {
  ...
  'ops-table': { icon: Users, title: 'No data available', description: 'There is no data to display at this time.' },
...
export function EmptyState({
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center py-16 px-4 text-center', className)}>
      ...
      <h3 className="text-lg font-semibold font-serif text-foreground mb-2">{title ?? defaults.title}</h3>
      ...
      {actionLabel && onAction && !actionHref && (
        <Button onClick={onAction} variant="outline" className="rounded-xl border-border/30">
          {actionLabel}
        </Button>
      )}
    </div>
