...
// Reusable tenant-aware email templates for transactional emails.
...
<body style="margin:0;padding:0;background:${BG_COLOR};font-family:Inter,system-ui,sans-serif;">
  <div style="max-width:560px;margin:0 auto;padding:32px 24px;">
    <div style="text-align:center;margin-bottom:24px;">
      <span style="font-size:20px;font-weight:700;color:${BRAND_COLOR};">${escapeHtml(siteName)}</span>
    </div>
    ${content}
    <div style="margin-top:32px;padding-top:16px;border-top:1px solid ${CARD_BG};text-align:center;">
      <p style="font-size:12px;color:${MUTED_COLOR};margin:0;">
        ${escapeHtml(siteName)} &middot; ${escapeHtml(siteTagline)}<br/>
        <a href="${siteUrl}" style="color:${BRAND_COLOR};text-decoration:none;">${escapeHtml(siteUrl)}</a><br/>
        <a href="mailto:${supportEmail}" style="color:${BRAND_COLOR};text-decoration:none;">${escapeHtml(supportEmail)}</a>
      </p>
    </div>
  </div>
</body>
  ...
  return {
    ...
    html: baseTemplate(`
      ...
      <div style="text-align:center;margin-top:24px;">
        <a href="${buildSiteUrl('/account')}" style="display:inline-block;background:${BRAND_COLOR};color:${BG_COLOR};font-weight:600;font-size:14px;padding:12px 24px;border-radius:8px;text-decoration:none;">View Your Order</a>
      </div>
    `),
  };
}
...
export function welcomeEmail(
  ...
  return {
    subject: `Welcome to ${siteName}, ${safeName}!`,
    html: baseTemplate(`
      <h1 style="color:${TEXT_COLOR};font-size:22px;margin:0 0 8px;">Welcome, ${safeName}!</h1>
      <p style="color:${MUTED_COLOR};font-size:14px;margin:0 0 24px;">You've joined the ${escapeHtml(siteName)} community. Here's what you can do:</p>
      <ul style="color:${TEXT_COLOR};font-size:14px;padding-left:20px;">
        ...
        <li style="margin-bottom:8px;">Complete quests to grow your ${escapeHtml(loyaltyProgramName)} balance</li>
      ...
      <div style="text-align:center;margin-top:24px;">
        <a href="${buildSiteUrl('/rewards')}" style="display:inline-block;background:${BRAND_COLOR};color:${BG_COLOR};font-weight:600;font-size:14px;padding:12px 24px;border-radius:8px;text-decoration:none;">Explore Rewards</a>
      </div>
    `),
  };
}
